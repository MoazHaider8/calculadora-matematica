...
const chips = ['Probabilidad simple', 'Casos favorables', 'Complementaria', 'Unión', 'Intersección'];

export function ArticuloBlogProbHero() {
  return (
    <section className="relative overflow-hidden bg-ocean" aria-label="Qué es la probabilidad y cómo se calcula">
      ...
      <div className="site-shell relative py-10 lg:py-14">
        <nav aria-label="Migas de pan" className="mb-6">
          <ol className="flex flex-wrap items-center gap-1.5 text-xs font-semibold">
            ...
            <li aria-hidden="true" style={{ color: 'rgba(221,243,240,0.35)' }}>&rsaquo;</li>
            <li style={{ color: '#DDF3F0' }}>Qué es la probabilidad</li>
          </ol>
        </nav>
        ...
        <div className="mt-7">
          <Link href="/calculadoras/estadistica/calculadora-de-probabilidad" className="inline-flex items-center gap-2 rounded-lg bg-white px-5 py-2.5 text-sm font-bold text-ink transition-colors hover:bg-gold">
            Usar calculadora de probabilidad &rarr;
          </Link>
        </div>
      </div>
    </section>
