...
type VarianzaSuccess = {
  type: 'success';
  values: number[];
  n: number;
  sum: number;
  mean: number;
  squaredDiffs: { value: number; diff: number; sq: number }[];
  sumSqDiffs: number;
  popVariance: number;
...
type VarianzaResult = VarianzaSuccess | { type: 'error'; message: string };

function compute(raw: string): VarianzaResult {
  if (!raw.trim()) return { type: 'error', message: 'Introduce al menos un valor para calcular la varianza.' };
  ...
  const mean = sum / n;
  const squaredDiffs = values.map(v => ({ value: v, diff: v - mean, sq: (v - mean) ** 2 }));
  const sumSqDiffs = squaredDiffs.reduce((a, d) => a + d.sq, 0);
  ...
  return { type: 'success', values, n, sum, mean, squaredDiffs, sumSqDiffs, popVariance, sampleVariance, popStdDev, sampleStdDev, procedure };
}
...
const EXAMPLES = [
  ...
  { label: 'Baja dispersión',  raw: '8, 8, 9, 9, 10'     },
  { label: 'Alta dispersión',  raw: '2, 10, 18, 26, 34'  },
  ...
  return (
    ...
    >
      ...
      <p
        ...
        style={{ color: highlight ? '#D8A31A' : '#102a43', fontSize: highlight ? '1.8rem' : '1.1rem' }}
      >
        {value}
      </p>
    </div>
...
export function VarianzaCalculator() {
  const [raw,      setRaw]      = useState(DEF_RAW);
  const [calcType, setCalcType] = useState<CalcType>('ambas');
  const [result,   setResult]   = useState<VarianzaResult>(DEF_RESULT);
        ...
        <div className="grid gap-8 lg:grid-cols-2">
          ...
          <div className="academic-card p-6">
            ...
            <div className="mb-5">
              ...
              <div className="grid grid-cols-3 gap-1" role="tablist" aria-label="Tipo de varianza">
                {TYPE_LABELS.map(t => (
                  <button
                    key={t.id}
                    role="tab"
            ...
            {/* Textarea */}
            <div className="mb-3">
              <label htmlFor="datos-var-input" className="mb-1.5 block text-xs font-bold text-ink">
              ...
              <textarea
                ...
                aria-label="Lista de datos para calcular la varianza"
              />
            </div>
            ...
            <div className="mb-5 rounded-xl px-4 py-3" style={{ background: '#0a3535' }}>
              <p className="mb-2 text-[9px] font-bold uppercase tracking-widest" style={{ color: 'rgba(221,243,240,0.45)' }}>Fórmulas</p>
              <div className="flex flex-col gap-1.5 font-mono text-xs" style={{ color: 'rgba(216,163,26,0.85)' }}>
                {showPop && <span>&sigma;&sup2; = &Sigma;(x &minus; &mu;)&sup2; / n</span>}
                {showSam && (
                  <span style={{ color: showPop ? 'rgba(216,163,26,0.6)' : 'rgba(216,163,26,0.85)' }}>
                    s&sup2; = &Sigma;(x &minus; x&#x0304;)&sup2; / (n&minus;1)
                  </span>
                )}
              </div>
            </div>
          ...
          <div className="academic-card overflow-hidden" aria-live="polite">
            ...
            <div className="border-b border-line px-7 py-4" style={{ background: '#F0FAF9' }}>
              <p className="text-sm font-bold" style={{ color: '#0a4f4d' }}>Resultado de varianza</p>
            </div>
            ...
            {result.type === 'error' ? (
              ...
              <div className="divide-y divide-line">
                ...
                <div className="px-7 py-4">
                  <p className="mb-1.5 text-[10px] font-bold uppercase tracking-widest text-muted">Media</p>
                ...
                {/* Squared differences */}
                <div className="px-7 py-4">
                  <p className="mb-2 text-[10px] font-bold uppercase tracking-widest text-muted">Diferencias cuadradas</p>
                  <div className="flex flex-col gap-0.5">
                    {(result.squaredDiffs.length <= 6 ? result.squaredDiffs : result.squaredDiffs.slice(0, 5)).map((d, i) => (
                      <p key={i} className="font-mono text-xs" style={{ color: '#334e68' }}>
                        ({fmt(d.value)} &minus; {fmt(result.mean)})&sup2; = {fmt(d.sq)}
                      </p>
                    ))}
                    {result.squaredDiffs.length > 6 && (
                      <p className="text-[10px] text-muted">...y {result.squaredDiffs.length - 5} más</p>
                    )}
                  </div>
                  <div
                  ...
                  >
                    <span className="text-[10px] font-bold uppercase tracking-widest" style={{ color: '#0a4f4d' }}>Suma:</span>
                    <span className="font-mono text-sm font-bold" style={{ color: '#0a4f4d' }}>{fmt(result.sumSqDiffs)}</span>
                  </div>
                </div>

                {/* Primary variance results */}
                <div className="px-7 py-5">
                  <p className="mb-3 text-[10px] font-bold uppercase tracking-widest text-teal">Varianza</p>
                  <div className="grid gap-3">
                    {showPop && (
                      <StatCard label="Poblacional (σ²)" value={fmt(result.popVariance)} highlight />
                    )}
                    {showSam && (
                      result.sampleVariance !== null ? (
                        <StatCard label="Muestral (s²)" value={fmt(result.sampleVariance)} highlight={!showPop} />
                      ) : (
                        <div className="rounded-xl border border-dashed border-line px-4 py-3" style={{ background: '#F8FAFC' }}>
                          <p className="text-[10px] font-bold uppercase tracking-widest text-muted">Muestral (s²)</p>
                          <p className="mt-1 text-xs text-muted">Se necesitan al menos 2 valores para calcular la varianza muestral.</p>
                        </div>
                      )
                    )}
                  </div>
                ...
                {/* Std dev support */}
                <div className="px-7 py-4">
                  <p className="mb-2 text-[10px] font-bold uppercase tracking-widest text-muted">Desviación estándar relacionada</p>
                  <div className="flex flex-col gap-1">
                    {showPop && (
                      <p className="font-mono text-xs" style={{ color: '#334e68' }}>
                        &sigma; = &radic;{fmt(result.popVariance)} = {fmt(result.popStdDev)}
                      </p>
                    )}
                    {showSam && result.sampleStdDev !== null && (
                      <p className="font-mono text-xs" style={{ color: '#334e68' }}>
                        s = &radic;{fmt(result.sampleVariance!)} = {fmt(result.sampleStdDev)}
                      </p>
                    )}
                  </div>
                </div>
                ...
                <div className="px-7 py-4">
                  ...
                  <p className="text-sm leading-relaxed text-slate">
                    {result.popVariance === 0
                      ? 'Todos los datos son iguales. La varianza es 0, no hay dispersión.'
